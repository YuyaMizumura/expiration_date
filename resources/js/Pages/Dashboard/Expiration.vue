    ...
    <AuthenticatedLayout>
        <template #header>
            <div class="flex justify-between">
                <div class="dummy"></div>
                <h2 class="font-semibold text-xl leading-tight">賞味期限</h2>
                <div>
        ...
        <div class="pt-3 selectDateWrap">
            <Link :href="route('dashboard.expiration', 
                {
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                <div class="overflow-hidden sm:rounded-lg">
                    <table>
                        ...
                        <tbody>
                            <tr v-for="(item, key) in props.expirations" v-if="props.expirations.length !== 0" :key="key">
                                <td>
                                    <div class="date">
                                ...
                                <td>
                                    <p :style="{ color: item.until_day.color }">{{ item.until_day.day }}日</p>
                                </td>
                                <td>
                                    {{ item.expense_date }}
                                </td>
                                <td>
                                    <a class="ml-5" :href="route('dashboard.complete', { 'id': key, 'u_id': mineId })">
        ...
        <BottomTabWrap>
            <BottomSignupBtn />
            <BottomTabBtn :left="{'name':'支出管理', 'url':'dashboard'}" :right="{'name':'賞味期限', 'url':'dashboard.expiration'}" :activeBotton="2" />
        </BottomTabWrap>

        <searchParts :status="searchFlg" :closeSearch="closeSearch" :searchAry="searchAry" :getAry="getAry" :url="'dashboard.expiration'" />
...
<style scoped>
    ...
    td .btn { display: flex; justify-content: center; align-items: center; }
</style>